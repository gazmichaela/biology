(function () {
  class DropdownManager {
    ...
    _onContentFocusIn(e) {
     /* this.state.hasFocusInside = true;*/
    ...
    _onContentFocusOut(e) {
      ...
      setTimeout(() => {
        ...
        if (!isFocusInContent && !isFocusInToggle && !isFocusInSubToggle && !isFocusInSubContent) {
         /* this.state.hasFocusInside = false;*/
    ...
    _onSubContentFocusIn(e) {
     /* this.state.hasSubFocusInside = true;*/
    ...
    _onSubContentFocusOut(e) {
      ...
      setTimeout(() => {
        ...
        if (!isFocusInSubToggle && !isFocusInSubContent) {
         /* this.state.hasSubFocusInside = false;*/
          
          if (this.elements.subContent) {
            const allElementsInSubMenu = this.elements.subContent.querySelectorAll('a, button, span, [tabindex]');
    ...
    _createSubDeadzone() {
      ...
      /*this.subDeadzoneElement.style.backgroundColor = 'rgba(255, 0, 0, 0.3)';*/
     ...
     /* this.subDeadzoneElement.style.border = '2px solid red';*/
      
      document.body.appendChild(this.subDeadzoneElement);
    }

    _updateSubDeadzone() {
      if (!this.subDeadzoneElement || !this.elements.subToggle || !this.elements.subContent) {
        return;
      }
    ...
    _isMouseInSubDeadzone() {
      if (!this.subDeadzoneElement || this.subDeadzoneElement.style.display === 'none') {
        return false;
      }
    ...
    _setupSubDropdown() {
      ...
      if (localStorage.getItem(this.config.storageKeys.isSubMenuOpen) === "true") {
        this.state.isClickOpenedSub = true;
        ...
        if (!this.state.isMouseOverMenuSub && !this._isMouseInSubDeadzone()) {
          ...
          if (currentElementBeforeClose) {
            ...
            if (isInSubContent) {
              ...
              const newIndex = this.state.focusableElements.indexOf(currentElementBeforeClose);
              if (newIndex !== -1) {
                this.state.currentFocusIndex = newIndex;
              }
            }
    ...
    _createDeadzone() {
     ...
     /* this.deadzoneElement.style.backgroundColor = 'rgba(255, 0, 0, 0.3)';*/
...
toggle.addEventListener("blur", () => {
  localStorage.removeItem(`${this.id}-toggleFocused`);
      ...
      if (container) {
        ...
        if (mainButton) {
          mainButton.addEventListener("focus", (e) => {
            const allDropdowns = document.querySelectorAll('.dropdown-content, .dropdown-content-second, .dropdown-content-third, .sub-dropdown-content');
            allDropdowns.forEach(dropdown => {
              const allElements = dropdown.querySelectorAll('a, button, span, [tabindex]');
      ...
      const allFocusableElements = content.querySelectorAll("a, button, input, select, textarea, [tabindex]:not([tabindex='-1'])");
    ...
    _bindSubDropdownEvents() {
      ...
      const subFocusableElements = subContent.querySelectorAll("a, button, input, select, textarea, [tabindex]:not([tabindex='-1'])");
