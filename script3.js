...
document.addEventListener("DOMContentLoaded", function () {
    ...
    dropdownContent2.style.left = "789px"; // Posun o 50px od levé strany
});


document.addEventListener("DOMContentLoaded", function () {
    const dropdownToggle = document.querySelector(".dropdown-toggle");
    const dropdownContent = document.querySelector(".dropdown-second-content");
  
    function positionSubmenu() {
      const buttonRect = dropdownToggle.getBoundingClientRect(); // Získá pozici tlačítka
      const buttonBottom = buttonRect.bottom; // Získáme pozici dolní části tlačítka
      const buttonLeft = buttonRect.left; // Získáme pozici levého okraje tlačítka
  
      // Nastavíme pozici podmenu podle tlačítka
      dropdownContent.style.top = `${buttonBottom + window.scrollY}px`; // Umístí podmenu přímo pod tlačítko
      dropdownContent.style.left = `${buttonLeft}px`; // Zarovná podmenu s tlačítkem
    }
  
    // Zavoláme tuto funkci při načítání stránky
    positionSubmenu();
  
    // Zavoláme tuto funkci, když se velikost okna změní
    window.addEventListener("resize", positionSubmenu);
  
    // Ujistíme se, že podmenu bude skryté, když opustíme tlačítko
    dropdownToggle.addEventListener("mouseenter", function () {
      dropdownContent.style.display = "block"; // Zobrazí podmenu při hoveru
    });
  
    dropdownToggle.addEventListener("mouseleave", function () {
      dropdownContent.style.display = "none"; // Skryje podmenu při opuštění hoveru
    });
  
    dropdownContent.addEventListener("mouseleave", function () {
      dropdownContent.style.display = "none"; // Skryje podmenu, když opustíme podmenu
    });
  });
